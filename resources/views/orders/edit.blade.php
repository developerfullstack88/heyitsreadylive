...
<div class="row" id="createOrderPage">
    ...
    @endif
	  <section class="card">
		  <header class="card-header font-title">@lang('add_order.edit_heading')</header>
			<div class="card-body">
				<div class="row ml-1 d-none">
						{{Form::label('',trans('add_order.does_customer_use_app'))}}
				</div>
        <form method="POST" action="{{route('orders.update-order-detail',$orderInfo->id)}}" accept-charset="UTF-8" id="orderForm">
          {{Form::token()}}
          <input type="hidden" name="_method" value="PUT"/>
	          ...
	          <div class="form-group">
							...
							{{Form::button(trans('setting.delete_complete_yes_option'),["class"=>"btn btn-default order-app-button order-app-button-first"])}}
						</div>
						<div class="form-group ml-2">
							...
							{{Form::button(trans('setting.delete_complete_no_option'),["class"=>"btn btn-default order-app-button order-app-button-last"])}}
						</div>
	        </div>
          <div class="form-group order-create-element">
						<div class="row">
							<div class="col-md-6">
	            	{{Form::label('order_number', trans('dashboard.table_th_order_number'))}}
		            {{Form::text('order_number',$orderInfo->order_number,
		              ['class'=>'form-control text-primary','placeholder'=>'','required'=>true,'readonly'=>true]
							...
							<div class="col-md-4">
								...
								{{Form::text('phone_number',$phoneNumber,
					...
					<div class="form-group order-create-element d-none">
						<div class="row">
          ...
          <div class="form-group order-create-element">
            ...
            {{Form::text('name',$orderInfo->user->name,
              ...
              ['class'=>'form-control','placeholder'=>trans('add_order.placeholder_email')]
            	...
            	{{Form::label('amount', trans('dashboard.table_th_amount'))}}
						  ...
						  <div class="input-group-prepend">
							...
							@endphp
							{{Form::text('amount',($orderInfo->amount)?$orderInfo->amount:'',
								['class'=>'form-control','placeholder'=>trans('add_order.placeholder_amount'),
      ...
      <div class="form-group form-check text-center order-create-element">
					<a href="{{route('home')}}" class="btn btn-lg btn-default"/>@lang('common.cancel_btn')</a>
          {{Form::submit(trans('add_order.update_order_btn'),["class"=>"btn btn-lg btn-primary hey-blue"])}}
      </div>
    </form>
		</section>
	</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//$('#amount').maskNumber();
    var callBtnVal={!! $orderInfo->call_btn !!}
    if(callBtnVal==0){
      $('.order-create-element').show();
      $('.order-app-button-first').attr('class','btn btn-primary order-app-button');
      var noBtn=$('.order-app-button-first').parents('.form-group').next().find('.order-app-button');
      ...
      $('.order-create-element').show();
      $('.order-app-button-last').attr('class','btn btn-info order-app-button');

      var yesBtn=$('.order-app-button-last').parents('.form-group').prev().find('.order-app-button');
      yesBtn.attr('class','btn btn-default order-app-button');
      $('.order-app-button-last').parents('.form-group').find('.radio-div input').prop('checked',true);
